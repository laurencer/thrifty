...
trait Importer extends (String => Option[FileContents]) {
  private[thrifty] def canonicalPaths: Seq[String] // for tests
   ...
   */
  private[thrifty] def getResolvedPath(filename: String): Option[String] = None
}
...
case class DirImporter(dir: File) extends Importer {
  private[thrifty] def canonicalPaths = Seq(dir.getCanonicalPath) // for test
  ...
  override private[thrifty] def getResolvedPath(filename: String): Option[String] = {
