...
import com.rouesnel.thrifty.ast._

class ThriftParserSpec extends Specification with ThrownExpectations {
  "ThriftParser" should {
    ...
    def verifyCommentParsing(source: String) =
      parser.parse(source, parser.rhs) must beEqualTo(IntLiteral(300))
...
"""
      parser.parse(source, parser.document) match {
        case Document(List(),List(Struct(SimpleID("MyStruct", None), "MyStruct", List(),
          None, m))) if m.isEmpty =>
        case x => ko(s"Failed to match $x")
      }
      ok
    }

    "double-quoted strings" in {
      parser.parse(""" "hello!" """, parser.rhs) must beEqualTo(StringLiteral("hello!"))
      parser.parse(""" "hello\nthere!" """, parser.rhs) must beEqualTo(StringLiteral("""hello\nthere!"""))
      parser.parse(""" "hello\\nthere!" """, parser.rhs) must beEqualTo(StringLiteral("""hello\\nthere!"""))
      parser.parse(""" "hello//there!" """, parser.rhs) must beEqualTo(StringLiteral("""hello//there!"""))
      parser.parse(""" "hello'there!" """, parser.rhs) must beEqualTo(StringLiteral("""hello'there!"""))
      parser.parse(""" "hello\'there!" """, parser.rhs) must beEqualTo(StringLiteral("""hello\'there!"""))
      parser.parse(""" "hello\"there!" """, parser.rhs) must beEqualTo(StringLiteral("""hello\"there!"""))
      parser.parse(""" "\"" """, parser.rhs) must beEqualTo(StringLiteral("\\\""))
    }
    ...
    "constant" in {
      parser.parse("300.5", parser.rhs) must beEqualTo(DoubleLiteral(300.5))
      parser.parse("cat", parser.rhs) must beEqualTo(IdRHS(SimpleID("cat")))
      val list = parser.parse("[ 4, 5, ]", parser.rhs)
      list.isInstanceOf[ListRHS] must beTrue
      list.asInstanceOf[ListRHS].elems.toList must beEqualTo(List(IntLiteral(4), IntLiteral(5)))
      parser.parse("{ 'name': 'Commie', 'home': 'San Francisco', }",
        parser.rhs) must beEqualTo(MapRHS(Seq(StringLiteral("name") -> StringLiteral
        ("Commie"), StringLiteral("home") -> StringLiteral("San Francisco"))))
    }

    "base types" in {
      parser.parse("i16", parser.fieldType) must beEqualTo(TI16)
      parser.parse("i32", parser.fieldType) must beEqualTo(TI32)
      parser.parse("i64", parser.fieldType) must beEqualTo(TI64)
      parser.parse("byte", parser.fieldType) must beEqualTo(TByte)
      parser.parse("double", parser.fieldType) must beEqualTo(TDouble)
      parser.parse("string", parser.fieldType) must beEqualTo(TString)
      parser.parse("bool", parser.fieldType) must beEqualTo(TBool)
      parser.parse("binary", parser.fieldType) must beEqualTo(TBinary)
    }

    "compound types" in {
      parser.parse("list<i64>", parser.fieldType) must beEqualTo(ListType(TI64, None))
      parser.parse("list<list<string>>", parser.fieldType) must beEqualTo(ListType(ListType(TString,
        None), None))
      parser.parse("map<string, list<bool>>", parser.fieldType) must beEqualTo(MapType(TString,
        ListType(TBool, None), None))
      parser.parse("set<Monster>", parser.fieldType) must beEqualTo(SetType(ReferenceType(Identifier("Monster")),
        None))
      parser.parse("Monster", parser.fieldType) must beEqualTo(ReferenceType(Identifier("Monster")))
    }

    "functions" in {
      parser.parse("/**doc!*/ void go()", parser.function) must beEqualTo(
      ...
      parser.parse(
        ...
        parser.function) must beEqualTo(
    ...
    "const" in {
      parser.parse("/** COMMENT */ const string name = \"Columbo\"", parser.definition) must beEqualTo(ConstDefinition(SimpleID("name"),
...
"""
      parser.parse(code, parser.definition) must beEqualTo(ConstDefinition(SimpleID("tyrion"),
...
"""
    ...
    "enum" in {
      val code = """
        ...
        """
      parser.parse(code, parser.definition) must beEqualTo(Enum(SimpleID("Direction"), Seq(
...
enum Foo
{
  ...
  Y = 2
      ...
      parser.parse(withComment, parser.enum) must beEqualTo(Enum(SimpleID("Foo"),
        Seq(
    ...
    "senum" in {
      // wtf is senum?!
      parser.parse("senum Cities { 'Milpitas', 'Mayfield' }", parser.definition) must beEqualTo(
      ...
      val code = """
        ...
        struct Point {
          ...
          multiline="also supported",
                 ...
                 """
      parser.parse(code, parser.definition) must beEqualTo(Struct(SimpleID("Point"), "Point", Seq(
    ...
    "union" should {
        ...
        val code = """
          ...
          union Aircraft {
            ...
            4: LighterThanAir lta
                   ...
                   """
        parser.parse(code, parser.definition) must beEqualTo(Union(SimpleID("Aircraft"), "Aircraft", Seq(
      ...
      "requiredness" in {
        ...
        laxParser.parse(code, laxParser.definition) must beEqualTo(Union(SimpleID("Aircraft"), "Aircraft", Seq(
          Field(1, SimpleID("a"), "a", ReferenceType(Identifier("Airplane")), None, Requiredness.Default),
    ...
    "exception" in {
      parser.parse("exception BadError { 1: string message }", parser.definition) must beEqualTo(
        Exception_(SimpleID("BadError"), "BadError",
          Seq(Field(1, SimpleID("message"), "message", TString, None, Requiredness.Default)), None))
      parser.parse("exception E { string message, string reason }", parser.definition) must beEqualTo(
      ...
      parser.parse("exception NoParams { }", parser.definition) must beEqualTo(
        Exception_(SimpleID("NoParams"), "NoParams", Seq(), None))
      parser.parse("/** doc rivers */ exception wellDocumentedException { }", parser.definition) must beEqualTo(
      ...
      parser.parse("exception BadError { 1: string message } (persisted = \"true\")", parser.definition) must beEqualTo(
        Exception_(SimpleID("BadError"), "BadError",
    ...
    "service" in {
      val code = """
        ...
        service Cache {
          ...
          binary get(1: string name) throws (1: NotFoundException ex);
                 ...
                 """
      parser.parse(code, parser.definition) must beEqualTo(Service(SimpleID("Cache"), None, Seq(
      ...
      parser.parse("service LeechCache extends Cache {}", parser.definition) must beEqualTo(
        Service(
          SimpleID("LeechCache"),
      ...
      val code = """
        ...
        service NullService {
          ...
          void doNothing();
                 ...
                 """
      parser.parse(code, parser.document) must beEqualTo(Document(
        ...
        """
          struct Airport {
            ...
            3: optional string name(postid="varchar(255)")
             ...
             sql_name="airports",)
        """
      parser.parse(code, parser.definition) must beEqualTo(
        Struct(
          SimpleID("Airport"),
          "Airport",
          Seq(
    ...
    "boolean default values" in {
      ...
      field.default must beEqualTo(Some(BoolLiteral(false)))

      field = parser.parse("bool x = 1", parser.field)
      field.default must beEqualTo(Some(BoolLiteral(true)))
      ...
      parser.parse("const bool z = false", parser.const) must beEqualTo(
        ConstDefinition(SimpleID("z", None), TBool, BoolLiteral(false), None))

      parser.parse("const bool z = True", parser.const) must beEqualTo(
      ...
      parser.parse("struct asdf { bool x = false }", parser.struct) must beEqualTo(
        Struct(SimpleID("asdf", None), "asdf",
          List(Field(-1, SimpleID("x", None), "x", TBool, Some(BoolLiteral(false)),
            Requiredness.Default, Map(), Map())),
          None,Map()))

      parser.parse("struct asdf { bool x = 1 }", parser.struct) must beEqualTo(
        Struct(SimpleID("asdf", None), "asdf",
  ...
  private def getParserForFilenameTest(thriftFilename: String): ThriftParser = {
    val importer = new Importer {
      ...
      override private[thrifty] def canonicalPaths: Seq[String] = Nil
      override def lastModified(filename: String): scala.Option[Long] = None
      override private[thrifty] def getResolvedPath(filename: String): scala.Option[String] = Some(filename)
    }
    new ThriftParser(importer, true)
  }


  def intercept[T](blk: => Any)(implicit ev: Manifest[T]): T = try {
    blk
    throw new Exception("Did not throw an exception")
  } catch {
    case ex: T => ex
    case ex: Exception => throw new Exception(s"Expected ${ev.runtimeClass.getName} exception. Instead got $ex", ex)
  }
}
