...
class TypeResolverSpec extends Specification {
  "TypeResolver" should {
    ...
    "resolve a known type" in {
      resolver(enumRef) must beEqualTo(enumType)
    }
        ...
        case ResolvedDefinition(enum2, resolver2) =>
          resolver2(struct, None) match {
            case ResolvedDefinition(struct2: Struct, _) =>
              struct2.fields(3).fieldType must beEqualTo(enumType)
              ok
            case _ =>
              ko
          }
    ...
    "not break on Void" in {
      resolver(Void) must beEqualTo(Void)
    }

    "transform a Field" in {
      val field = Field(42, SimpleID("foo"), "foo", structRef)
      resolver(field) must beEqualTo(field.copy(fieldType = structType))
    }
    ...
    "transform a TypeDef" in {
      ...
      resolver(typedef, None).definition must beEqualTo(
        typedef.copy(fieldType = enumType))
    }

    "transform a Struct" in {
      resolver(struct, None).definition must beEqualTo(struct.copy(fields = struct.fields.map(resolver.apply)))
    }

    "transform an Exception" in {
      resolver(ex, None).definition must beEqualTo(ex.copy(fields = ex.fields.map(resolver.apply)))
    }

    "transform a Const" in {
      val const = ConstDefinition(SimpleID("foo"), enumRef, IdRHS(Identifier("SomeEnum.FOO")), None)
      resolver(const, None).definition must beEqualTo(ConstDefinition(SimpleID("foo"), enumType, EnumRHS(enum, foo), None))
    }
    ...
    "const definition transitivity" in {
      ...
      newResolver(copy, None).definition must beEqualTo(
    ...
    "allow a valid MapRHS for a StructType" in {
      ...
      value must beEqualTo(StructRHS(sid = structType2.sid, elems = structElems))
    }
    ...
    "transform a Service" in {
      ...
      resolver(service, None).definition must beEqualTo(service.copy(functions = Seq(resolver(fun))))
    }
    ...
    "resolve a service parent from same scope" in {
      ...
      resolver(service2, None).definition must beEqualTo(
        service2.copy(
    ...
    "resolve a parameter from an included scope" in {
      ...
      resolveFieldType.asInstanceOf[StructType].scopePrefix must beEqualTo(Some(SimpleID("other")))
    }
    ...
    "resolve a service parent from an included scope" in {
      ...
      val subSvc = Service(
        ...
        Nil,
        None)
      resolver(subSvc, None).definition must beEqualTo(
    ...
    "resolve a typedef from an included scope" in {
      ...
      resolvedDoc.defs(0) match {
        case Struct(_, _, fields, _, annotations) => {
          fields(0) match {
            case Field(1, _, _, ListType(StructType(_, Some(SimpleID("typedef1", _))), None), _, _, _, _, _) => ok
            case _ => ko
          }
          fields(1) match {
            case Field(2, _, _, SetType(StructType(_, Some(SimpleID("typedef1", _))), None), _, _, _, _, _) => ok
            case _ => ko
          }
          annotations must beEqualTo(Map("foo" -> "bar"))
        }
        case _ =>
          ko
      }
    ...
    "include positions in type errors" should {
      "TypeNotFoundException" in {
        ...
        ex.node.pos.line must beEqualTo(1)
        ex.node.pos.column must beEqualTo(7)
      }
      ...
      "UndefinedConstantException" in {
        ...
        ex.node.pos.line must beEqualTo(1)
        ex.node.pos.column must beEqualTo(15)
      }
      ...
      "UndefinedSymbolException" in {
        ...
        ex.node.pos.line must beEqualTo(2)
        ex.node.pos.column must beEqualTo(19)
      }
